...
import com.nic.publictax.adapter.PendingAdapter;
...
import com.nic.publictax.databinding.PendingScreenBinding;
...
public class PendingScreen extends AppCompatActivity implements Api.ServerResponseListener {
    ...
    public class fetchPendingtask extends AsyncTask<Void, Void,
        ...
        protected ArrayList<PublicTax> doInBackground(Void... params) {
            dbData.open();
            ArrayList<PublicTax> publicTaxes = new ArrayList<>();
            publicTaxes = dbData.getSavedPMAYDetails();
            Log.d("PMAY_COUNT", String.valueOf(publicTaxes.size()));
            return publicTaxes;
        }

        @Override
        protected void onPostExecute(ArrayList<PublicTax> publicTaxes) {
            super.onPostExecute(publicTaxes);
            recyclerView.setVisibility(View.VISIBLE);
            pendingAdapter = new PendingAdapter(PendingScreen.this, publicTaxes);
