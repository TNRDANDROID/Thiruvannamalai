...
import com.nic.publictax.Fragment.SlideshowDialogFragment;
import com.nic.publictax.R;
import com.nic.publictax.adapter.FullImageAdapter;
...
import com.nic.publictax.databinding.FullImageRecyclerBinding;
...
public class FullImageActivity extends AppCompatActivity implements View.OnClickListener, Api.ServerResponseListener {
    ...
    private static  ArrayList<PublicTax> activityImage = new ArrayList<>();
    private com.nic.publictax.dataBase.dbData dbData = new dbData(this);
    ArrayList<PublicTax> imageListTree;
...
//        work_id = ;
    ...
    public class fetchImagetask extends AsyncTask<Void, Void,
            ArrayList<PublicTax>> {
        @Override
        protected ArrayList<PublicTax> doInBackground(Void... params) {
            ...
            PublicTax imageTree = new PublicTax();
            PublicTax imageTree1 = new PublicTax();
        ...
        @Override
        protected void onPostExecute(final ArrayList<PublicTax> imageList) {
    ...
    public void generateImageArrayList(JSONArray jsonArray){
        if(jsonArray.length() > 0){
            ...
            for(int i = 0; i < jsonArray.length(); i++ ) {
                try {
                    PublicTax imageOnline = new PublicTax();
                    PublicTax imageOnline1 = new PublicTax();
