...
import com.nic.publictax.databinding.MainPageBinding;
...
public class MainPage extends AppCompatActivity implements Api.ServerResponseListener ,MyDialog.myOnClickListener{
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        try {
                            ...
                            for (int j=0;j<jsonArray1.length();j++){
                                ...
                                publicTax.setFin_year(jsonArray1.getJSONObject(j).getString("fin_year"));
                                dbData.insert_Master_Fin_Year(publicTax);
                            }
                            for (int j=0;j<jsonArray2.length();j++){
                                PublicTax publicTax = new PublicTax();
                                publicTax.setWork_code(jsonArray2.getJSONObject(j).getInt("work_code"));
                                publicTax.setWork_name(jsonArray2.getJSONObject(j).getString("work_name"));
                                dbData.insert_Master_Work_Type(publicTax);
                            }
                            for (int j=0;j<jsonArray3.length();j++){
                                PublicTax publicTax = new PublicTax();
                                publicTax.setShg_code(jsonArray3.getJSONObject(j).getInt("shg_code"));
                                publicTax.setShg_name(jsonArray3.getJSONObject(j).getString("shg_name"));
                                dbData.insert_Master_Self_Help_Group(publicTax);
                            }
                            for (int j=0;j<jsonArray4.length();j++){
                                PublicTax publicTax = new PublicTax();
                                publicTax.setShg_code(jsonArray4.getJSONObject(j).getInt("shg_code"));
                                publicTax.setShg_member_code(jsonArray4.getJSONObject(j).getInt("shg_member_code"));
                                publicTax.setMember_name(jsonArray4.getJSONObject(j).getString("member_name"));
                                dbData.insert_Master_Self_Help_Group_Member(publicTax);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
    ...
    public void syncButtonVisibility() {
        dbData.open();
        ArrayList<PublicTax> workImageCount = dbData.getAllTreeImages();
    ...
    public void logout() {
        dbData.open();
        ArrayList<PublicTax> ImageCount = dbData.getAllTreeImages();
        if (!Utils.isOnline()) {
